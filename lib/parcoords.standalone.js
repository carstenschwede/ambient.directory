...
(function (global, factory) {
    ...
    var computeControlPoints = function computeControlPoints(smoothness, centroids,d) {
      ...
      if (!d.randomYOffset) {
        window.globalStringToRandom = window.globalStringToRandom || {};
        if (d.idx) {
          window.globalStringToRandom[d.idx] = window.globalStringToRandom[d.idx] || Math.random();
        }
        d.randomYOffset = 30*(0.5-window.globalStringToRandom[d.idx]);
      }

      cps.forEach((cp,i) => {
        cp.elements[1] = Math.max(cp.elements[1],10);
        let yOffset = new Vector([0,d.randomYOffset]);
        //let yOffset = new Vector([0,20*(0.5-Math.random())]);
        cps[i] = cp.add(yOffset);
      })
    ...
    var singleCurve = function singleCurve(config, position, d, ctx) {
      ...
      var cps = computeControlPoints(config.smoothness, centroids,d);
    ...
    var DefaultConfig = {
      ...
      nullValueSeparatorPadding: { top: 16, right: 0, bottom: 8, left: 0 },
      color: '#069',
