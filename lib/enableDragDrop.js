					...
					for(let entry of entries) {
						try {
							...
							if (!result.citationKey) {
								...
								result.citationKey = unidecode(result.lastAuthor.toLowerCase() + result.title.toLowerCase().replace(/\W+/g,x => x == "-" ? x : " ").trim().split(" ").filter(x => !!x && titleIgnoreWords.indexOf(x) == -1).slice(0,3).map(k => toCamelCase(k).replace("-","")).join("") + result.year);
