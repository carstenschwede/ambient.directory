...
let addCrossRef = async function(entry) {
	...
	try {
		if (entry.entryTags.doi) {
			...
			result = (await queryCrossRef("?query.bibliographic="+encodeURIComponent(entry.entryTags.title)+"&rows=5&select=DOI,title,author,issued,container-title,is-referenced-by-count")).message.items[0];
		}
		console.log(result);
