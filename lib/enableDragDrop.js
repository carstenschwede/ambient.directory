					...
					for(let entry of entries) {
						try {
							...
							if (!result.citationKey) {
								let titleIgnoreWords = ["in","of","the","is","out"];
								result.citationKey = result.lastAuthor.toLowerCase() + result.title.toLowerCase().split(/\W/).filter(x => !!x && titleIgnoreWords.indexOf(x) == -1).slice(0,3).map(k => toCamelCase(k)).join("") + result.year;
