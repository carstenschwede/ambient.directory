					...
					for(let entry of entries) {
						try {
							...
							if (!result.citationKey) {
								let titleIgnoreWords = ["a","in","of","the","is","out","for","to","and"];

								//Generate CitationKey as
								//lowercase LASTAUTHOR + camelcased(first 3 non-trivial words in title) + year
								result.citationKey = unidecode(result.lastAuthor.toLowerCase().replace(/\s+/g,"") + result.title.toLowerCase().replace(/\W+/g,x => x == "-" ? x : " ").trim().split(" ").filter(x => !!x && titleIgnoreWords.indexOf(x) == -1).slice(0,3).map(k => toCamelCase(k).replace("-","")).join("") + result.year);
