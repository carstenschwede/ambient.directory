...
(async () => {
	...
	let columns = Object.entries(tableFields).map(([field, properties],i) => {
		let result = {};
		result.idx = i;
		result.data = field;
		...
		let limitedChoice = result.editable && (_.isArray(result.editable) || _.isObject(result.editable));
		if (limitedChoice) {
			let [mainField,subField] = field.split(".");
			transformed.forEach(row => {
				row[mainField][subField] = row[mainField][subField].toUpperCase();
			});
		}
	...
	//transformed = transformed.slice(0,500);
	...
	if (haveLocalChangesBeenApproved) {
		UIkit.modal.alert('Some of your local changes have been approved remotely. Thanks for your contributions!')
		saveDBUpdates();
		updateChangeButtons();
	}

	if (haveLocalChangesBeenDiscarded) {
		UIkit.modal.alert('Some of your local changes were discarded, most likely because the publication you have made changes to has been removed from the directory in the meantime.')
		saveDBUpdates();
		updateChangeButtons();
	}
	...
	$(document).ready(function () {
		...
		//AutoFilter
		window.columnsById = columnsById;
		window.table = table;
		...
		ths.forEach(th => {
			...
			/*
			if (!columnDesc.searchable) return console.warn("NOT SEARCHABLE");
			*/
			...
			if (columnDesc.filter == "*") {
				...
				Object.entries(colValues).sort((a,b) => {
					if (a[1].label.indexOf("[") == 0) return 10;
					if (b[1].label.indexOf("[") == 0) return 10;
